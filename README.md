...
- **图片拖拽/粘贴插入** — 直接拖拽图片到编辑器或粘贴截图，自动保存到文档专属文件夹
- **链接智能打开** — 预览区点击网页链接用系统浏览器打开，点击本地文件链接用系统默认程序打开
...
- **拖拽/粘贴插入** — 直接拖拽图片文件到编辑器，或粘贴系统截图，自动保存并插入 Markdown 语法
- **文档专属图片文件夹** — 每个 `.md` 文档对应一个 `[文档名].images/` 文件夹，避免多文档图片混淆，删除文档时一并清理
- **自动路径解析** — 预览区自动将本地图片路径转为应用内可访问 URL，图片即时可见
...
│   ├── main.ts             # 编辑器、标签页、同步滚动、主题、图片插入、链接处理
│   ├── styles.css          # 全部 UI 样式
│   └── assets/             # 静态资源
├── src-tauri/              # Tauri / Rust 后端
│   ├── src/lib.rs          # 文件 I/O、图片保存、macOS 集成命令
│   ├── icons/              # 应用图标（自动生成脚本）
...
- **拖拽/粘贴监听** — 在 CodeMirror 编辑器 DOM 上监听 `drop` 和 `paste` 事件，识别图片 MIME type 和文件扩展名
- **后端二进制保存** — 通过 Tauri `invoke("save_image")` 将图片数据写入磁盘，自动创建文件夹
- **文档专属文件夹** — 根据当前文档文件名生成 `[文档名].images/` 文件夹，避免多文档图片混杂
- **预览区路径转换** — 利用 Tauri `assetProtocol` 和 `convertFileSrc`，将本地文件路径转为 WebView 可访问的安全 URL，图片即时渲染

### 链接智能处理

预览区渲染完成后，为所有 `<a>` 标签绑定点击事件：
- `http://` / `https://` / `mailto:` / `tel:` → 调用系统浏览器/邮件客户端
- 本地相对/绝对路径 → 解析为完整路径后通过系统默认程序打开
- 相对路径基于当前文档所在目录自动解析
